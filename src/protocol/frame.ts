...
export function buildFrame(command: number, data: number[] = []): number[] {
...
export function checkFrameStructure(bytes: number[]): FrameError | null {
...
export function validateFrame(bytes: number[]): FrameError | null {
...
export function parseFrame(bytes: number[]): ParsedFrame {
  const structural = checkFrameStructure(bytes);
  if (
    structural === FrameError.TOO_SHORT ||
  ...
  const crc =
    (bytes[bytes.length - CRC_SIZE] << 8) | bytes[bytes.length - CRC_SIZE + 1];
  return {
