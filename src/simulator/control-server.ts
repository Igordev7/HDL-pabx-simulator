...
export class SimulatorControlServer {
  private static instancia?: SimulatorControlServer;
  private server?: http.Server;

  private constructor(private readonly cb: PainelCallbacks) {}
  ...
  private start(): void {
    ...
    server.on('error', (err: Error) => {
      logger.warn(
        `[SIM][CTRL] Painel de controle indisponível: ${err.message}`
      );
    ...
    server.listen(porta, '127.0.0.1', () => {
      const p = (server.address() as AddressInfo | null)?.port ?? porta;
  ...
  private abrirNoNavegador(url: string): void {
  ...
  private handle(
    ...
    const inteiro = (nome: string, padrao: number): number =>
      Number(url.searchParams.get(nome)) || padrao;
    const flag = (nome: string, padrao: boolean): boolean => {
    ...
    const comSimulador = (fn: (sim: NonNullable<ReturnType<typeof getSimuladorAtivo>>) => unknown): void => {
      ...
      json(200, { ok: true, ...(fn(sim) as object) });
    };
    ...
    switch (rota) {
      ...
      case '/receber-config': {
        ...
        if (p.has('ramais')) patch.qtdRamais = Number(p.get('ramais'));
        if (p.has('inicial')) patch.ramalInicial = Number(p.get('inicial'));
        if (p.has('porteiros')) patch.qtdPorteiros = Number(p.get('porteiros'));
        if (p.has('troncos')) patch.troncos = Number(p.get('troncos'));
        if (p.has('senha')) patch.senhaProgramador = Number(p.get('senha'));
        setConfigRecebimento(patch);
        if (p.has('replay')) setCaminhoReplay(p.get('replay') ?? '');
        if (flag('resetestado', false)) limparProgEstado();
        json(200, {
          ok: true,
