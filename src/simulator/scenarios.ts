 ...
 */
function ramalParaBytes(numeroFixo: number): [number, number] {
...
function frameDiscagem(opts: {
  ...
  ramalFixo: number;
  ramalFixoA?: number | null;
  subEstadoByte?: number;
}): number[] {
  const data = new Array<number>(8).fill(0); // alcança frame[10] => data[7]
...
export function frameLigacaoToca(origemFixo: number, destinoFixo: number): number[] {
...
export function frameLigacaoConversa(
  origemFixo: number,
  destinoFixo: number
): number[] {
  return frameRamalTdi({
...
export function frameLigacaoDesliga(destinoFixo: number): number[] {
  return frameRamalTdi({
...
export function cenarioLigacao(
  origemFixo = 201,
  destinoFixo = 204,
  atende = true,
  duracaoSeg = 5
  ...
  const toca: PassoCenario = {
    emMs: 0,
    frame: frameLigacaoToca(origemFixo, destinoFixo),
  };
  const desliga = (emMs: number): PassoCenario => ({
    emMs,
 ...
 */
export function cenarioAcionamentoFechadura(
  porteiroFixo = 200,
  ramalFixo = 205,
  fechadura: 1 | 2 | 3 = 1
): PassoCenario[] {
 ...
 */
export function cenarioAcessoPorteiro(
  porteiroFixo = 200,
  ramalFixo = 205
): PassoCenario[] {
  return [
 ...
 */
export function cenarioAcessoSenha(
  porteiroFixo = 200,
  ramalFixo = 205
): PassoCenario[] {
  return [
 ...
 */
export function cenarioAlerta(zonaFixo = 205, ativado = true): PassoCenario[] {
  return [
    {
      emMs: 0,
      frame: frameDiscagem({
 ...
 */
export function cenarioAlarme(zonaFixo = 205, disparado = true): PassoCenario[] {
  return [
    {
      emMs: 0,
      frame: frameDiscagem({
 ...
 */
function frameProgRamalCategoria(
  r: RamalSimulado,
  ov?: RamalOverride
): number[] {
  const flex = ov?.flexivel ?? r.flexivel ?? r.fixo;
  const wordFisico = Math.max(0, r.fixo - 200);
  const p = new Array<number>(44).fill(0); // frame 49 bytes >= 47
  ...
  if (ov?.desvioModo && ov.desvioModo !== 'desativado') {
    const tipo =
      ov.desvioModo === 'sempre'
        ? 0x00
        : ov.desvioModo === 'ocupado'
          ? 0x10
          : 0x30; // externo
    const destFisico =
      ov.desvioDestinoFixo != null ? Math.max(0, ov.desvioDestinoFixo - 200) : 0;
...
const CONFIG_DUMP_PADRAO: ConfigDump = {
  qtdRamais: 11,
  ramalInicial: 200,
  qtdPorteiros: 0,
  troncos: 4,
...
export function ramaisDoDump(cfg: ConfigDump): RamalSimulado[] {
  const lista: RamalSimulado[] = [];
