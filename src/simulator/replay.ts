 ...
 */
export function lerFramesDaCaptura(caminho: string): number[][] {
  let texto: string;
  try {
    texto = readFileSync(caminho, 'utf8');
  } catch (err) {
    logger.warn(
      `[SIM][REPLAY] Não consegui ler "${caminho}": ${(err as Error).message}`
    );
    return [];
  }
 ...
 */
export function cenarioReplayCaptura(caminho: string): PassoCenario[] | null {
  ...
  const passos: PassoCenario[] = frames.map((frame, i) => ({
    emMs: 200 + i * 30,
    frame,
  }));
  if (!temFim) {
  ...
  logger.info(
    `[SIM][REPLAY] ${frames.length} frames de programação da captura "${caminho}".`
  );
  return passos;
}
