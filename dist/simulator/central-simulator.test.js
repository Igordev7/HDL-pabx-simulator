...
import { setModeloSimulado, getModeloSimulado, setRespostaEnvio, setConfigRecebimento, setChamadaAtiva, } from './runtime.js';
...
function framesDoComando(emitidos, cmd) {
...
describe('CentralSimulator — handshake', () => {
    ...
    it('INFO_INSTALACAO: ecoa de volta o Código/Ramal que o CTI mandou (nada inventado)', async () => {
        ...
        sim.onBytesFromCti(buildFrame(ComandosPABX.INFO_INSTALACAO, [0xff, 0xff, 0xff]));
        await vi.advanceTimersByTimeAsync(50);
...
describe('CentralSimulator — alerta / alarme / programações', () => {
    ...
    it('round-trip: EFE_PROGRAMACAO PROG_RML_FLEXIVEL reflete no dump do Receber seguinte', async () => {
        ...
        sim.onBytesFromCti(buildFrame(ComandosPABX.EFE_PROGRAMACAO, [0x37, 0x20, 0x92, 0x91, 0xff]));
        await vi.advanceTimersByTimeAsync(50);
        sim.onBytesFromCti(buildFrame(ComandosPABX.SOL_PROGRAMACAO, [0x00]));
        await vi.advanceTimersByTimeAsync(3000);
        const cats = framesDoComando(emitidos, ComandosPABX.INF_PROGRAMACAO).filter((f) => f[3] === ProgramacaoPABX.PROG_RML_CATEGORIA);
...
//# sourceMappingURL=central-simulator.test.js.map